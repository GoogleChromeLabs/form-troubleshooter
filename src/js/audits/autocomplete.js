...
export function hasAutocompleteAttributes(tree) {
  ...
  const invalidFields = findDescendants(tree, INPUT_SELECT_TEXT_FIELDS)
    .filter(node => !node.attributes.autocomplete && node.attributes.type !== 'hidden'
...
export function hasAutocompleteOff(tree) {
  ...
  const issues = [];
  const invalidFields = findDescendants(tree, INPUT_SELECT_TEXT_FIELDS)
    .filter(node => node.attributes.autocomplete && node.attributes.autocomplete.trim() === 'off');

  if (invalidFields.length) {
    issues.push({
      details: 'Found form field(s) with <code>autocomplete="off"</code>:<br>• ' +
        `${invalidFields.map(field => stringifyFormElementAsCode(field)).join('<br>• ')}<br>Although <code>autocomplete="off"</code> is
