...
import {hasAutocompleteAttributes, hasAutocompleteOff, hasEmptyAutocomplete, hasValidAutocomplete} from './autocomplete';

describe('autocomplete', function() {
  describe('hasAutocompleteAttributes', function() {
    it('should not return audit error when input has autocomplete', function() {
      const tree = getTreeNodeWithParents({children: [{name: 'input', attributes: {name: 'input1', autocomplete: 'cc-name'}}]});
  ...
  describe('hasValidAutocomplete', function() {
    ...
    it('should return audit error when input has invalid autocomplete value (multiple tokens)', function() {
      const tree = getTreeNodeWithParents({children: [{name: 'input', attributes: {autocomplete: 'stuff username'}}]});
      const result = hasValidAutocomplete(tree);
      expect(result.length).to.equal(1);
    ...
    it('should not return audit error when input has valid autocomplete value (multiple tokens)', function() {
      const tree = getTreeNodeWithParents({children: [{name: 'input', attributes: {autocomplete: 'shipping address-line1'}}]});
      const result = hasValidAutocomplete(tree);
      expect(result).to.be.eql([]);
    });
    ...
    it('should not return audit error when input has valid autocomplete value (section address)', function() {
      const tree = getTreeNodeWithParents({children: [{name: 'input', attributes: {autocomplete: 'section-1 address-line1'}}]});
      const result = hasValidAutocomplete(tree);
      expect(result).to.be.eql([]);
    });
